...
export function renderInvoicePdf(order, res) {
  ...
  for (const item of items) {
    ...
    doc.text(`${item.title}`, 50, rowY, { width: 250 });
    doc.text(`by ${item.author}`, 50, rowY + 13, { width: 250 });
    doc.text(String(item.quantity), 320, rowY);
    doc.text(`$${item.price.toFixed(2)}`, 380, rowY);
    doc.text(`$${(item.price * item.quantity).toFixed(2)}`, 470, rowY);
    rowY += 40;
  }

  doc.moveTo(50, rowY).lineTo(545, rowY).strokeColor('#e3d9c6').stroke();
  rowY += 15;
  ...
  doc.fontSize(12).fillColor('#1c2b39').text(`Total due: $${total}`, 380, rowY);
  rowY += 50;
  ...
  const acceptedAt = order.tos_accepted_at
    ...
    : 'N/A';
  doc.text(`Agreed to Terms of Service: ${acceptedAt}`, 50, rowY + 35);

  if (order.signature && order.signature.startsWith('data:image')) {
    const base64 = order.signature.split(',')[1];
    const imgBuffer = Buffer.from(base64, 'base64');
    doc.text('Customer signature:', 50, rowY + 55);
    try {
      doc.image(imgBuffer, 50, rowY + 70, {
        width: 180,
        height: 70,
        fit: [180, 70],
      });
    } catch {
      doc.text('(signature could not be rendered)', 50, rowY + 75);
    }
  } else {
    doc.text('Customer signature: not on file', 50, rowY + 55);
  }
  ...
  doc
    ...
    .fillColor('#aaa')
    .text(
      'This invoice was generated automatically by Bindery & Co.',
      50,
      rowY + 160
    );
